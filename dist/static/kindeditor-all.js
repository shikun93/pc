...
//自己添加
	function _imageUpLoad(){
		var t = this;
		$(".ke-upload-area").ajaxSubmit({
			url : $(t).attr("data-url"), // 请求的url
			type : "post", // 请求方式
			async :true, // 异步
			success : function(imageUrl){
				var result = JSON.parse(imageUrl);
				if(result.data.status =="success"){
					if (!K.IE) {
						document.getElementsByClassName("ke-edit-iframe")[0].contentWindow.document.designMode = "on";
						document.getElementsByClassName("ke-edit-iframe")[0].contentWindow.document.contentEditable = true;
					}
					var img  = document.createElement("img");
					img.src=result.data.url;
					//img.width =document.getElementById("onlyImgWidth").value;
					if(_IE){
						imageSelfIframe.contentEditable = true;
					}
					imageSelfIframe.body.appendChild(img);
					document.body.removeChild(document.getElementsByClassName("ke-dialog-default")[0]);
					if(document.getElementsByClassName("ke-dialog-mask")[0]){
						document.body.removeChild(document.getElementsByClassName("ke-dialog-mask")[0]);
					}
				}
			},
			error : function(){
...
function _createButton(arg,type) {
	arg = arg || {};
	var name = arg.name || '',
		span = K('<span class="ke-button-common ke-button-outer" title="' + name + '"></span>'),
		btn = K('<input class="ke-button-common ke-button" data-url="' + arg.uploadJson + '" type="button" value="' + name + '" />');
	//自己添加
	if (name == "确定") {
		if(type=="link"){
			btn.click(arg.click);
...
_extend(KDialog, KWidget, {
	init : function(options) {
		...
		_each([
			...
			if (this.btn) {
				//自己修改
				var button = _createButton(this.btn,self.name);
...
KindEditor.plugin('image', function(K) {
	...
	self.plugin.imageDialog = function(options) {
		...
		var html = [
			...
			hiddenElements.join(''),
			//自己添加
			'<input type="input" style="display:none" name="admin_token" value="' +sessionStorage.getItem("admin_token")  + '" />',
			'<input type="input" style="display:none" name="'+self.data.name+'" value="'+self.data.value+'" />',
			...
			'<label style="width:60px;">图片宽度 </label>',
			'<input id="onlyImgWidth" type="text" class="ke-input-text" style="width:50px;" />'	,
			'</form>',
			'</div>',
			//local upload - end
			'</div>'
			...
			var dialog = self.createDialog({
				...
				yesBtn : {
					...
					click : function(e) {
						...
						if (border !== '') {
							html += ' border="' + border + '"';
							html += ' class="table table-bordered"';
						}
						if (border === '' || border === '0') {
